...
function x = solve_hybrid(interval_m)
    ...
    [rows_np, cols_np] = size(non_parametric_system);   
    ...
    % Solve the system
    x = solve(size(inf(interval_m),1), center, ...
        radius, [b2 b1],size(padded_non_parametric_system,2));
end

function x = solve(vars_count, center, mg, b, cols_par)
    ...
    A = [-eye(vars_count), zeros(vars_count, cols_par - vars_count);
